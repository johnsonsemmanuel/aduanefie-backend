...
class EquipmentController extends Controller
{
    public function index(Request $request)
    {
        ...
        $equipment = Equipment::with('item.store')
            ->when($storeId, fn ($q) => $q->whereHas('item', fn ($itemQ) => $itemQ->where('store_id', $storeId)))
            ->when($status && in_array($status, ['available', 'maintenance', 'retired']), fn ($q) => $q->where('status', $status))
            ->when($search, fn ($q) => $q->whereHas('item', fn ($itemQ) => $itemQ->where('name', 'like', "%{$search}%")))
            ->orderBy('created_at', 'desc')
            ->paginate(config('default_pagination'));

        $stores = Store::orderBy('name')->get(['id', 'name']);

        return view('admin-views.equipment.index', compact('equipment', 'status', 'search', 'storeId', 'stores'));
    }

    public function create()
    {
        $stores = Store::with('module')->orderBy('name')->get(['id', 'name', 'module_id']);
        $units = Unit::orderBy('created_at')->get(['id', 'unit']);

        return view('admin-views.equipment.create', compact('stores', 'units'));
    }
    ...
    {
        $equipment = Equipment::with('item.store')->findOrFail($id);
        $stores = Store::with('module')->orderBy('name')->get(['id', 'name', 'module_id']);
        $units = Unit::orderBy('created_at')->get(['id', 'unit']);
        $categories = Category::where(['position' => 0])->module($equipment->item->store->module_id)->get();

        return view('admin-views.equipment.edit', compact('equipment', 'stores', 'units', 'categories'));
    }
    ...
    public function update(Request $request, $id)
    {
        $request->validate([
            ...
            'store_id' => 'required|exists:stores,id',
            'category_id' => 'required|exists:categories,id',
            'image' => 'nullable|image|max:' . MAX_FILE_SIZE * 1024,
            'stock' => 'required|integer|min:1',
            'hourly_rate' => 'nullable|numeric|min:0',
            'daily_rate' => 'nullable|numeric|min:0',
            'weekly_rate' => 'nullable|numeric|min:0',
        ...
        $equipment = Equipment::with('item')->findOrFail($id);
        $store = Store::findOrFail($request->store_id);
