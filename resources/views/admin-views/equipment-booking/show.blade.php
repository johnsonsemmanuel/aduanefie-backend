    ...
    <div class="content container-fluid">
        ...
        <div class="row g-2">
            ...
            <div class="col-lg-4">
                <div class="card h-100">
                    <div class="card-header">
                        <h5 class="card-title m-0">{{ translate('Store') }}</h5>
                    </div>
                    <div class="card-body">
                        @if ($booking->store)
                            <div class="font-weight-semibold mb-2">{{ $booking->store->name }}</div>
                            <div class="text-muted">{{ $booking->store->phone }}</div>
                            <div class="text-muted">{{ $booking->store->address }}</div>
                        @else
                            <p class="mb-0">{{ translate('Unavailable') }}</p>
                        @endif
                    </div>
                    ...
                    <div class="card-body">
                        <div class="btn--container justify-content-start flex-wrap">
                            @if ($booking->status === 'pending')
                                <form action="{{ route('admin.equipment-bookings.confirm', $booking->id) }}" method="post">
                                    @csrf
                                    <button type="submit" class="btn btn--primary">
                                        <i class="tio-checkmark-circle"></i> {{ translate('Confirm Booking') }}
                                    </button>
                                </form>
                                <form action="{{ route('admin.equipment-bookings.cancel', $booking->id) }}" method="post">
                                    @csrf
                            ...
                            @elseif ($booking->status === 'confirmed')
                                <form action="{{ route('admin.equipment-bookings.mark-active', $booking->id) }}" method="post">
                                    @csrf
                                    <button type="submit" class="btn btn--primary">
                            ...
                            @elseif (in_array($booking->status, ['active', 'overdue']))
                                <form action="{{ route('admin.equipment-bookings.mark-returned', $booking->id) }}" method="post" class="d-flex align-items-end flex-wrap gap-2">
                                    @csrf
                                    <div>
