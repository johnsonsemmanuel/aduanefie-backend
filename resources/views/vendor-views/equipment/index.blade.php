    ...
    <div class="content container-fluid">
        <div class="page-header">
            <div class="row align-items-center">
                ...
                <div class="col-sm-auto">
                    <a class="btn btn-sm btn--primary" href="{{ route('vendor.equipment.create') }}">
        ...
        <div class="js-nav-scroller hs-nav-scroller-horizontal mt-2 mb-3">
            <ul class="nav nav-tabs border-0 nav--tabs">
                <li class="nav-item">
                    <a class="nav-link {{ $status === null ? 'active' : '' }}" href="{{ route('vendor.equipment.index') }}">
                        {{ translate('All') }}
                    </a>
                </li>
                @foreach(['available', 'maintenance', 'retired'] as $equipmentStatus)
                    <li class="nav-item">
                        <a class="nav-link {{ $status === $equipmentStatus ? 'active' : '' }}"
                           href="{{ route('vendor.equipment.index', ['status' => $equipmentStatus]) }}">
                                ...
                                <tr>
                                    ...
                                    <td class="table-column-pl-0">
                                        <div class="d-flex align-items-center gap-2">
                                            ...
                                            <a href="{{ route('vendor.equipment.edit', $item->id) }}" class="text-dark font-weight-semibold">
                                                {{ $item->item?->name }}
                                            </a>
                                        </div>
                                    </td>
                                        ...
                                        <div class="d-flex gap-1">
                                            @if ($item->status === 'available')
                                                <a class="btn action-btn btn-outline-warning"
                                                   href="{{ route('vendor.equipment.status', [$item->id, 'maintenance']) }}"
                                                   title="{{ translate('Mark Under Maintenance') }}">
                                                    <i class="tio-settings-outlined"></i>
                                                </a>
                                            @elseif ($item->status === 'maintenance')
                                                <a class="btn action-btn btn-outline-success"
                                                   href="{{ route('vendor.equipment.status', [$item->id, 'available']) }}"
                                                   title="{{ translate('Mark Available') }}">
                                                    <i class="tio-checkmark-circle"></i>
                                                </a>
                                            @endif
                                            <a class="btn action-btn btn--warning btn-outline-warning"
                                               href="{{ route('vendor.equipment.edit', $item->id) }}"
                                               title="{{ translate('Edit') }}">
                                                <i class="tio-edit"></i>
                                            </a>
                                            <a class="btn action-btn btn--danger btn-outline-danger"
                                               href="javascript:void(0)"
                                               onclick="route_alert('{{ route('vendor.equipment.destroy', $item->id) }}', '{{ translate('Want to delete this equipment?') }}')"
