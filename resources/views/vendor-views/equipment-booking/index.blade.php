            ...
            <ul class="nav nav-tabs border-0 nav--tabs">
                <li class="nav-item">
                    <a class="nav-link {{ $status === null ? 'active' : '' }}"
                       href="{{ route('vendor.equipment-bookings.index') }}">
                        {{ translate('All') }}
                    </a>
                </li>
                @foreach(['pending', 'confirmed', 'active', 'overdue', 'completed', 'cancelled'] as $bookingStatus)
                    <li class="nav-item">
                        <a class="nav-link {{ $status === $bookingStatus ? 'active' : '' }}"
                           href="{{ route('vendor.equipment-bookings.index', ['status' => $bookingStatus]) }}">
                            ...
                            @foreach ($bookings as $key => $booking)
                                ...
                                <tr>
                                    ...
                                    <td class="table-column-pl-0">
                                        <a class="text-dark font-weight-semibold" href="{{ route('vendor.equipment-bookings.show', $booking->id) }}">
                                    ...
                                    <td>
                                        <a class="btn action-btn btn--warning btn-outline-warning"
                                           href="{{ route('vendor.equipment-bookings.show', $booking->id) }}"
